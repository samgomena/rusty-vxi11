...
fn wrap_error<T>(e: serde_xdr::EncoderError, desc: &str) -> io::Result<T> {
    match e {
        ...
        serde_xdr::EncoderError::Unknown(s) => Err(io::Error::new(
            io::ErrorKind::Other,
            format!("{}: {}", desc, s),
        )),
    }
...
impl<TApp: AppCodec> XdrCodec<TApp> {
    ...
    fn decode_call(&mut self, buf: &mut EasyBuf) -> io::Result<Option<()>> {
        ...
        match msg.body {
            xdr_rpc::Body::Call { cbody: body } => match body.rpcvers {
                2u32 => {
                    self.prog = body.prog;
                    self.vers = body.vers;
...
impl<TApp: AppCodec> Codec for XdrCodec<TApp> {
    ...
    fn decode(&mut self, buf: &mut EasyBuf) -> io::Result<Option<Self::In>> {
        while buf.len() > 0 {
            match self.state {
                XdrCodecState::AwaitingCall => match self.decode_call(buf) {
                    Ok(o) => match o {
                        Some(_) => {
                ...
                XdrCodecState::AwaitingBody => {
                    let val = self
                        .app_codec
                        .app_decode(self.prog, self.vers, self.proc_, buf);
                    return match val {
                        Ok(r) => {
                            self.state = XdrCodecState::AwaitingCall;
                            match r {
                                Some(v) => Ok(Some(xdr_rpc::XdrRequest {
                                    xid: self.xid,
                                    val: v,
                                })),
                                None => Ok(None),
    ...
    fn encode(&mut self, msg: Self::Out, buf: &mut Vec<u8>) -> io::Result<()> {
        ...
        serde_xdr::to_bytes(&reply, &mut scratch)?;
        self.app_codec.encode(msg.val, &mut scratch)?;
